...
class CrossAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, attn_pdrop=.1, resid_pdrop=.1):
        ...
        self.mlp_vis = nn.Sequential(nn.Linear(256, 2 *256),
                                     # nn.SiLU(),  # changed from GELU
                                     nn.GELU(),  # changed from GELU
                                     nn.Linear(2 * 256, 256),
                                     nn.Dropout(0.1),
                                     )
        self.mlp_ir = nn.Sequential(nn.Linear(256, 2 *256),
                                    # nn.SiLU(),  # changed from GELU
                                    nn.GELU(),  # changed from GELU
                                    nn.Linear(2 *256, 256),
                                    nn.Dropout(0.1),
...
class SelfCrossFcResnet(nn.Module):
    def __init__(self, depth,dropout= 0.0, num_classes=1000,):
        ...
        self.pool_max_1 = nn.AdaptiveMaxPool2d(output_size=(2, 2))
        self.pool_max_2 = nn.AdaptiveMaxPool2d(output_size=(2, 2))
        self.pool_max_3 = nn.AdaptiveMaxPool2d(output_size=(2, 2))
        self.pool_max_4 = nn.AdaptiveMaxPool2d(output_size=(2, 2))

        # 自注意力
        self.cross_attention = CrossAttention(256, 128, 0, 4)
    ...
    def forward(self, x):
        ...
        for stage in range(3):
            ...
            bs = temp1.shape[0]
            temp_1 = self.pool_max_3(temp1).view(bs, -1)
            temp_2 = self.pool_max_4(temp2).view(bs, -1)
            crossFusionKnowledge.append((torch.flatten(temp_1,1), torch.flatten(temp_2,1)))
        ...
        # 自注意力
        bs = x1.shape[0]
        tmp_x1 = self.pool_max_1(x1).view(bs, -1)
        tmp_x2 = self.pool_max_2(x2).view(bs, -1)
        x1_, x2_ = self.cross_attention(tmp_x1, tmp_x2)
