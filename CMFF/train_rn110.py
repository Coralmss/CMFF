...
parser.add_argument('--cu_num', default='4', type=str)
...
parser.add_argument('--widen_factor', type=int, default=110, help='Model width.')
...
model = network_Resnet.cross_resnet(num_classes=num_classes,
            depth=args.depth,)
...
# According to CIFAR
module = network_Resnet.Fusion_module(4096, num_classes, 1)
model.to(DEVICE)
module.to(DEVICE)
...
def train(model, module, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss = loss_cross + loss_kl+ 3*loss_net1_2  +  2*loss_sim
        ###################################################################################
        loss.backward()
