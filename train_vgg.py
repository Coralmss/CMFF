...
parser.add_argument('--exp_name', default='cifar100/vgg16', type=str)
...
model = network_VGG.cross_vgg16(num_classes=num_classes)
torch.save(model.state_dict(), 'save_vgg_new.pth') #保存模型参数
# According to CIFAR
# module = network_VGG.BRUModule(1024,num_classes=num_classes)
module = network_VGG.Fusion_module(512, num_classes, 1)
model.to(DEVICE)
module.to(DEVICE)
...
def eval(net, module, epoch, test_flag=False):
    ...
    if acc > topacc:
        torch.save(model.state_dict(), 'vgg_best.pth')  # 保存模型参数
        topacc = acc
...
def train(model, module, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss_sim  = F.cross_entropy(simi, sim_target)  #simi={Tensor:(384,128)}  sim_target={Tensor:(128,)}


        fused_logit = module(fmap[0].unsqueeze(-1).unsqueeze(-1), fmap[1].unsqueeze(-1).unsqueeze(-1))  #融合模块函数
        ...
        loss = loss_cross + loss_kl + loss_net1_2  + 5 * loss_sim #完整loss
        #loss = loss_cross + loss_kl+ loss_net1_2 #消融
...
if __name__ == '__main__':
    ...
    # Instantiate logger
    #logger = SummaryLogger(path)train_vgg_test.py
